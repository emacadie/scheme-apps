...
;
; from https://github.com/pkrumins/the-seasoned-schemer/blob/master/19-absconding-with-the-jewels.ss
; because, frankly, I got lost
; I think that somehow running things in tests is messing up the data
; perhaps I will look into this later
...
(module+ test
  ...
  (runit:check-equal? (deep 6) '((((((pizza)))))))
  (runit:check-equal? (six-layers 'pizza) '((((((pizza)))))))
  (runit:check-equal? (four-layers 'pizza) '((((pizza)))))
  ; (runit:check-equal? (deepB 6) '((((((pizza)))))))
  (runit:check-equal? (six-layers 'mozzarella) '((((((mozzarella)))))))
  (runit:check-equal? (toppings 'mozzarella)   '((((((mozzarella)))))))
  (runit:check-equal? (toppings 'cake)         '((((((cake)))))))
  (runit:check-equal? (toppings 'pizza)        '((((((pizza)))))))
  (runit:check-equal? (cons (toppings 'cake) '()) '((((((cake)))))))
