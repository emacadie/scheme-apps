...
(module+ test
  ...
  (printf "(c-to-f 23): ~a \n" (c-to-f 23))
  (check-equal? (c-to-f 23) 73.4 "Wrong answer for (c-to-f 23)")
  (printf "(f-to-c 55): ~a \n" (f-to-c 55))
  (check-equal? (f-to-c 55) 12.777777777777779 "Wrong answer for (f-to-c 55)")
  (printf "(fourth-with-square 4): ~a \n" (fourth-with-square 4))
  (check-equal? (fourth-with-square 4) 256 "Wrong answer for (fourth-with-square 4)")
  (printf "(simply-abs -4): ~a \n" (simply-abs -4))
  (check-equal? (simply-abs -4) 4 "Wrong answer for (simply-abs -4)")
  (printf "(scientific 7 3): ~a \n" (scientific 7 3))
  (check-equal? (scientific 7 3) 7000.0 "Wrong answer for (scientific 7 3)")
  (printf "(scientific 42 -5): ~a \n" (scientific 42 -5))
  (check-equal? (scientific 42 -5) 0.00042 "Wrong answer for (scientific 42 -5)")
  (printf "(discount 10 5): ~a \n" (discount 10 5))
  (check-equal? (discount 10 5) 9.5 "Wrong answer for (discount 10 5)")
  (printf "(discount 29.90 50): ~a \n" (discount 29.90 50))
  (check-equal? (discount 29.90 50) 14.95 "Wrong answer for (discount 29.90 50)")
  (printf "(tip 29.23): ~a \n" (tip 29.23))
  (check-equal? (tip 29.23) 4.77 "Wrong answer for (tip 29.23)")
  (printf "(tip 7.54): ~a \n" (tip 7.54))
  (check-equal? (tip 7.54) 1.46 "Wrong answer for (tip 7.54)")
)
